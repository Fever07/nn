...
class PGD_attack:
    def __init__(self, model, 
        ...
        self.logits = model(self.x_input)
        logits_correct_class = tf.reduce_sum(self.logits * y_onehot, axis=1)
    ...
    def generate_imgs_and_probs(self, sess, images, labels_or_targets, verbose=False):
        """ Generates adversarial images/
            sess: the tensorflow session
            images: a 4D tensor containing the original images
            labels_or_targets: for non-targeted attacks, the actual or predicted labels
